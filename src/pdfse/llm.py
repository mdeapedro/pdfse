...
async def fetch_heuristic(
    ...
    samples_data: Union[list[bytes], list[str]],
    image_mode: bool
) -> dict[str, list]:
    client = get_client()
    user_content: list[dict] = [
        {
            "type": "text",
            "text": f"""
            Here is the extraction schema for this task. Please generate the JSON heuristic based on this schema and the provided samples:
    ...
    if image_mode:
        # Handle image samples (list[bytes])
        for imageb in samples_data:
            base64_image_url = _encode_image_to_base64(imageb) # type: ignore
            user_content.append({
                "type": "image_url",
                "image_url": {
        ...
        # Handle text samples (list[str])
        for i, text_content in enumerate(samples_data):
            user_content.append({
                "type": "text",
                "text": f"""
                ---
                SAMPLE {i+1} (Text Content with Layout)
                ---
                {text_content}
                """
            })

    system_prompt = get_system_prompt(image_mode)
...
def get_system_prompt(image_mode: bool) -> str:
    if image_mode:
        return _SYSTEM_PROMPT_IMAGE_MODE
    else:
        return _SYSTEM_PROMPT_TEXT_MODE


_SYSTEM_PROMPT_TEXT_MODE = """You are an expert AI assistant specializing in PDF data extraction. Your task is to act as a 'heuristic generator' for a navigation robot.
...
You will receive an `extraction_schema` (JSON) and one or more `SAMPLE` blocks containing the **text content (with layout)** of a PDF. This text content preserves the layout (line breaks, reading order) of the original document.
...
1.  **Text Samples (Layout Context):** Blocks of text (e.g., `SAMPLE 1`, `SAMPLE 2`). The order of words and lines in this text *directly corresponds* to the layout and reading order. Use this to understand *proximity* and *relative positioning* of words.
...
1.  **CRITICAL: Handle Layout Variability.**
    * Documents with the same `label` can have **radically different layouts**. You might receive multiple text samples (e.g., 3 examples) that look different but share the same *label*.
...
* **Text Sample:**
    ```
    SAMPLE 1 (Text Content with Layout)
    ---
    SON GOKU
    (some other text)
    (some other text)
...
_SYSTEM_PROMPT_IMAGE_MODE = """You are an expert AI assistant specializing in PDF data extraction. Your task is to act as a 'heuristic generator' for a navigation robot.
